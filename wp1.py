...
def reverse_cipher():
	...
	while index <= end:  # loops when char count <= total chars in file   
		...
		index += 1
		if index <= end:  # if char count <= total chars in file (calls this everytime before .seek)
			...
			if char1 == b'\n' and char2 == b'\r':
				...
				index += 1
				if index <= end:
					input_file.seek(-1,1) #moves file pointer to left 1 again
			else:
				output_file.write(char1)  # if it's not \n & \r just write a single char
		else:  # 1 char left 
			output_file.write(char1)  # if it's not \n & \r just write a single char
...
def read_char(input_file):  # reads char w/out changing position of file pointer
